...
<head>
  ...
  <title>ViewTube | Dynamic Auth Portal</title>
  ...
  <style>
    * {
    ...
    .big-title span { color: var(--red); text-shadow: 0 0 40px var(--red-glow); }
    ...
    .deco-bar:nth-child(1) { height: 20px; }
    ...
    .input-group label { font-size: 0.75rem; color: #777; text-transform: uppercase; letter-spacing: 1.5px; }
    ...
    .input-wrap input:focus { border-color: var(--red); box-shadow: 0 0 0 3px rgba(255,0,0,0.1); }
    .field-icon {
      position: absolute;
      left: 14px;
      top: 50%;
    ...
    .btn-signin:hover, .btn-register:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(255,0,0,0.5); }
    .meta-row { display: flex; justify-content: space-between; align-items: center; margin: 1rem 0; font-size: 0.75rem; }
    .forgot-link { color: var(--red); text-decoration: none; }
    ...
    .or-divider { display: flex; align-items: center; gap: 0.8rem; margin: 1rem 0; color: #333; font-size: 0.75rem; }
    ...
    .strength-bar { height: 3px; flex: 1; background: #1a1a1a; border-radius: 2px; }
    .strength-bar.active-weak { background: #e53e3e; }
    .strength-bar.active-medium { background: #ed8936; }
    .strength-bar.active-strong { background: #48bb78; }
    .terms-row { display: flex; gap: 0.6rem; margin: 1rem 0; font-size: 0.78rem; align-items: center; }

    @media (max-width: 820px) {
      .twin-panels { flex-direction: column; }
      .left-panel, .right-panel { width: 100%; transform: none !important; }
      .center-rotator { display: none; }
    }
  </style>
</head>
<body>
  ...
  <div class="twin-panels" id="twinPanels">
    ...
    <div class="panel left-panel" id="leftPanel">
      <div class="branding-login" id="brandingLogin">
        ...
        <div class="deco-bars">
          <div class="deco-bar"></div><div class="deco-bar"></div><div class="deco-bar"></div>
          <div class="deco-bar"></div><div class="deco-bar"></div><div class="deco-bar"></div>
          <div class="deco-bar"></div><div class="deco-bar"></div>
        </div>
      </div>
    ...
    <div class="panel right-panel" id="rightPanel">
      ...
      <div class="form-card" id="loginFormCard">
        <div style="display: flex; align-items:center; gap:8px; margin-bottom:1rem;">
          <div style="background:#FF0000; width:32px; height:32px; border-radius:8px; display:flex; align-items:center; justify-content:center;"><svg width="16" height="16" viewBox="0 0 24 24" fill="white"><polygon points="5,3 19,12 5,21"/></svg></div>
          <span style="font-family: 'Bebas Neue'; font-size:1.5rem;">VIEW<span style="color:#FF0000;">TUBE</span></span>
        </div>
        <div class="form-header"><h2 style="font-family:'Bebas Neue';">Welcome Back</h2><p>Don't have an account? <a href="#" id="switchToRegisterLink" style="color:var(--red);">Create one →</a></p></div>
        <div class="divider" style="margin:1rem 0;"></div>
        <div class="error-msg" id="loginErrorMsg"><span id="loginErrorText">Invalid credentials</span></div>
        <form id="loginFormElem">
          <div class="input-group"><label>Email</label><div class="input-wrap"><input type="text" id="loginEmail" placeholder="john@example.com" required /><svg class="field-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg></div></div>
          <div class="input-group"><label>Password</label><div class="input-wrap"><input type="password" id="loginPassword" placeholder="Enter password" required /><svg class="field-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg><button type="button" class="pw-toggle" onclick="toggleLoginPassword()">👁️</button></div></div>
          <div class="meta-row"><div><input type="checkbox" id="rememberMe" /> <label for="rememberMe">Remember me</label></div><a href="#" class="forgot-link">Forgot password?</a></div>
          <button type="submit" class="btn-signin">▶ &nbsp; Sign In</button>
        </form>
        <p style="text-align:center; font-size:0.7rem; color:#333; margin-top:1.2rem;">Protected by ViewTube</p>
      </div>

      <!-- REGISTER FORM -->
      <div class="form-card" id="registerFormCard" style="display: none;">
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:1rem;"><div style="background:#FF0000; width:32px; height:32px; border-radius:8px; display:flex; align-items:center; justify-content:center;"><svg width="16" height="16" viewBox="0 0 24 24" fill="white"><polygon points="5,3 19,12 5,21"/></svg></div><span style="font-family:'Bebas Neue'; font-size:1.5rem;">VIEW<span style="color:#FF0000;">TUBE</span></span></div>
        <div class="form-header"><h2>Create Account</h2><p>Already have an account? <a href="#" id="switchToLoginLink" style="color:var(--red);">Sign in →</a></p></div>
        <div class="divider"></div>
        <form id="registerFormElem">
          <div class="input-group"><label>Full Name</label><div class="input-wrap"><input type="text" id="fullName" placeholder="John Doe" required /><svg class="field-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div></div>
          <div class="input-group"><label>Email Address</label><div class="input-wrap"><input type="email" id="regEmail" placeholder="hello@viewtube.com" required /><svg class="field-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg></div></div>
          <div class="input-group"><label>Password</label><div class="input-wrap"><input type="password" id="regPassword" placeholder="Min. 8 characters" oninput="checkRegStrength()" required /><svg class="field-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg><button type="button" class="pw-toggle" onclick="toggleRegPassword()">👁️</button></div><div class="password-strength" id="strengthBars"><div class="strength-bar" id="s1"></div><div class="strength-bar" id="s2"></div><div class="strength-bar" id="s3"></div><div class="strength-bar" id="s4"></div></div></div>
          <div class="input-group"><label>Confirm Password</label><div class="input-wrap"><input type="password" id="confirmPass" placeholder="Repeat password" required /><svg class="field-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M9 12l2 2 4-4"/><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div></div>
          <div class="terms-row"><input type="checkbox" id="termsCheckbox" required /><span>I agree to <a href="#">Terms</a> and <a href="#">Privacy</a>. I'm 13+.</span></div>
...
<script>
  ...
  document.getElementById('registerFormElem')?.addEventListener('submit', (e) => {
    ...
    setTimeout(() => {
      ...
      alert(`Welcome ${name}! Your channel is ready.`);
    }, 1500);
  });
